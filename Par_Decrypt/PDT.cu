...
cudaTextureObject_t tex_inverse_elements;  
...
__global__ void computeOut(cudaTextureObject_t tex_inverse_elements,unsigned char *d_ciphertexts, gf *d_images)
{
    __shared__ gf c[sb];
    ...
    if (tid < SYND_BYTES) {
        ...
        #pragma unroll 
        for (int bit = 0; bit < 8 && tid * 8 + bit < sb; ++bit) {
            c[tid * 8 + bit] = (r >> bit) & 1;
        }
    }
...
if (globalIdx < 2 * SYS_T) {
    ...
    // #pragma unroll 
    for (int i = 0; i < sb; ++i) {
        sum = add(sum, mul(tex1Dfetch<gf>(tex_inverse_elements, i * 2 * SYS_T + globalIdx), c[i]));
    }
    s_out[globalIdx] = sum;
}
    ...
    // Main loop of the Berlekamp-Massey algorithm 
...
void InitializeC() {
    ...
    // Setup texture memory for inverse elements
    ...
    resDesc.res.linear.devPtr = d_inverse_elements;
    resDesc.res.linear.desc.f = cudaChannelFormatKindUnsigned;
    resDesc.res.linear.desc.x = 16; // assuming gf is 32 bits
    ...
    cudaCreateTextureObject(&tex_inverse_elements, &resDesc, &texDesc, NULL);
}
...
int synd_f() {
    ...
    computeOut<<<numBlocks, threadsPerBlock>>>(tex_inverse_elements,d_ciphertexts, d_images);
